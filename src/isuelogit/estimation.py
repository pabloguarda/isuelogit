...
from isuelogit.printer import block_output, printProgressBar, enablePrint, blockPrint, printIterationBar
...
from isuelogit.etl import fake_observed_counts, masked_observed_counts, get_informative_links
from isuelogit.descriptive_statistics import summary_links_report, rmse, nrmse

from isuelogit.mytypes import ColumnVector, ParametersDict, Matrix, Proportion
from isuelogit.utils import get_design_matrix, v_normalization, is_pos_def, is_pos_semidef, almost_zero, Options
        ...
        for iter in np.arange(2, iters + 1, 1):
            ...
            if iter > 2:
                for i in range(iter - 1, 1, -1):
                    ...
                    d_error = np.array(report['d_error']).astype(float)[:, np.newaxis]
