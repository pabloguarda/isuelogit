...
if TYPE_CHECKING:
    from isuelogit.mytypes import Positions, Links, Nodes, Dict, Feature,ColumnVector, List, DataFrame, Tuple
    from isuelogit.links import Link

from isuelogit.utils import Options
import isuelogit.config as config
import isuelogit.geographer as geographer
...
class DataReader:
    ...
    def __init__(self, setup_spark = False, network_key:str = None):
        ...
        if setup_spark:
            ...
            # sfc, sqlContext = self.setup_spark_context(n_cores=1, ram_memory=1)
...
class SparkReader:
    ...
    # @blockPrinting
    def setup_spark_context(self, spark_home_path = None, java_home_path = None, n_cores=None, ram_memory=None):
        ...
        if spark_home_path is None:
            spark_home_path = \
                "/Users/pablo/Library/CloudStorage/OneDrive-Personal/university/cmu/1-courses/2021-1/10605-ml-with-ld/software/spark-3.0.1-bin-hadoop2.7"

        os.environ['SPARK_HOME'] = spark_home_path

        if java_home_path is None:
            # Path for JAVA installation
            java_home_path = "/Library/Java/JavaVirtualMachines/jdk-13.0.1.jdk/Contents/Home"
        ...
        # os.environ['PYSPARK_PYTHON'] = "/Users/pablo/Library/Caches/pypoetry/virtualenvs/gisuelogit-3EREI3WF-py3.8/bin/python"
        ...
        def setup_spark_context_with_warnings():
            ...
            self.findspark.init(spark_home = os.environ['SPARK_HOME'])
    ...
    def read_pems_counts_data(self,
                              filepath: str,
                              selected_period: {}) -> (pd.DataFrame, pd.DataFrame):
        ...
        # There are 5 lane dependent attributes according to the pems dictionary, and thus, each of the attributes is
        # computed under assumption of a maximum of 8 lanes (12 + 5 x 8 = 52)
...
def read_spatiotemporal_data_fresno(network,
    ...
    if build_fresno_gis_files:
        ...
        geographer.write_line_segments_shp(
            links=links,
            folderpath=config.dirs['output_folder'] + 'gis/Fresno/network/links/test',
            networkname='Fresno'
        )
