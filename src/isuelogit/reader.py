...
if TYPE_CHECKING:
    from isuelogit.mytypes import Nodes, Paths, Matrix,TNetwork, DiTNetwork, MultiDiTNetwork
...
from isuelogit.paths import Path, get_paths_od_from_paths
from isuelogit.networks import MultiDiTNetwork
...
def read_tntp_linkdata(network_name: str,
    ...
    Method uses data from the TNTP repo

    Arguments:

    - local_files: If False, data is read from tntp repository.

    - folderpath: path of the folder where a file ending with '_net' and with '.tntp' extension is located.
    These files can be found at the TNTP repo at https://github.com/bstabler/TransportationNetworks/tree/master
    ...
    Returns: a pandas dataframe with link data
...
def read_tntp_od(network_name: str,
    ...
    Returns: a 2-D numpy array with the OD matrix
