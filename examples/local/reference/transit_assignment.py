...
root = os.getcwd() + "/examples/data/networks/Github/"
subfolder = 'SiouxFalls/'
...
def updateTravelTime():
    ...
    for l in linkSet:
        ...
        linkSet[l].cost = linkSet[l].cost*theta['tt']
...
def computeLogLikelihood():
    ...
    for l in linkSet:
        if linkSet[l].reasonable == True: # If reasonable link
            linkSet[l].logLike = math.exp(nodeSet[l[1]].key - nodeSet[l[0]].key - linkSet[l].cost) # Label has shortest path value to node
...
assignment("stochastic", "MSA", accuracy = 0.01, maxIter=10)
writeUEresults()